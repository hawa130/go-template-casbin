...
// The PublicKeyFunc type is an adapter to allow the use of ordinary function as a Querier.
type PublicKeyFunc func(context.Context, *ent.PublicKeyQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f PublicKeyFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.PublicKeyQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.PublicKeyQuery", q)
}

// The TraversePublicKey type is an adapter to allow the use of ordinary function as Traverser.
type TraversePublicKey func(context.Context, *ent.PublicKeyQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraversePublicKey) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraversePublicKey) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.PublicKeyQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.PublicKeyQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.PublicKeyQuery:
		return &query[*ent.PublicKeyQuery, predicate.PublicKey, publickey.OrderOption]{typ: ent.TypePublicKey, tq: q}, nil
	case *ent.UserQuery:
